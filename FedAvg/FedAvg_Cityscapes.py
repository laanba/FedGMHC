"""
FedAvg_Cityscapes.py — 标准联邦平均（FedAvg）基线方法
                        专用于 Cityscapes 数据集

用途
----
作为 FedGMHC 方法的对比基线，使用相同的模型（MobileNetV2-UNet）、
...
结果保存
--------
每次运行结果统一保存在 result_save/FedAvg_MMDDHHmm/ 子目录下：
  result_save/
  └── FedAvg_MMDDHHmm/
...
def save_curves(global_history, run_dir):
    ...
    for ylabel, title, suffix, ydata in [
        ('Pixel Accuracy',
         'Cityscapes FedAvg — Pixel Accuracy vs. Round',
         'pixel_accuracy.png', pa_vals),
        ('mIoU',
         'Cityscapes FedAvg — mIoU vs. Round',
         'miou.png', miou_vals),
    ]:
        plt.figure(figsize=(12, 5))
        plt.plot(rounds, ydata,
                 linestyle='-', marker='o', linewidth=2, markersize=4,
                 color='steelblue', label='FedAvg Global')
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"训练配置（FedAvg Baseline — Cityscapes）:")
    ...
    print(f"\n{'='*80}")
    print(f"开始 FedAvg 训练（Cityscapes）— 标准基线")
    print(f"{'='*80}")
    ...
    for round_idx in range(NUM_ROUNDS):
        ...
        if g_miou > best_miou:
            ...
            torch.save({
                ...
                'target_size':      TARGET_SIZE,
                'method':           'FedAvg',
            }, os.path.join(save_dir, 'best_model_fedavg_cityscapes.pth'))
            print(f"  ★ Best (mIoU: {g_miou:.4f})")
        else:
            print()

        if (round_idx + 1) % 10 == 0:
            ckpt = os.path.join(save_dir, f'fedavg_cityscapes_round_{round_idx + 1}.pth')
            torch.save({
                'round':            round_idx + 1,
                'model_state_dict': global_model.state_dict(),
                'num_classes':      NUM_CLASSES,
    ...
    # ===== 打印训练总结 =====
    print(f"\n{'='*80}")
    print(f"训练完成！总结如下（FedAvg Baseline — Cityscapes）：")
    print(f"{'='*80}")
